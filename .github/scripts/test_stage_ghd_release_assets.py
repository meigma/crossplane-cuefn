...
class StageGhdReleaseAssetsTest(unittest.TestCase):
    def test_stages_expected_assets(self) -> None:
        with fixture() as root:
            ...
            self.assertEqual(
                ...
                [
                    "checksums.txt",
                    "cuefn_1.2.3_darwin_amd64",
                    "cuefn_1.2.3_darwin_amd64.sbom.json",
                    "cuefn_1.2.3_darwin_arm64",
                    "cuefn_1.2.3_darwin_arm64.sbom.json",
                    "cuefn_1.2.3_linux_amd64",
                    "cuefn_1.2.3_linux_amd64.sbom.json",
                    "cuefn_1.2.3_linux_arm64",
                    "cuefn_1.2.3_linux_arm64.sbom.json",
                ],
            )
            linux_binary = root / "dist/release-assets/cuefn_1.2.3_linux_amd64"
            mode = linux_binary.stat().st_mode
            self.assertTrue(mode & stat.S_IXUSR)
            self.assertIn("dist/release-assets/cuefn_1.2.3_linux_arm64", stdout)

    def test_fails_on_missing_checksum_entry(self) -> None:
        with fixture(missing_checksum="cuefn_1.2.3_linux_arm64") as root:
            result, _, stderr = run_script(root)

            self.assertEqual(result, 1)
            self.assertIn("missing checksum entry", stderr)
            self.assertIn("cuefn_1.2.3_linux_arm64", stderr)

    def test_fails_on_checksum_mismatch(self) -> None:
        override = ("cuefn_1.2.3_linux_amd64", "0" * 64)
        with fixture(checksum_override=override) as root:
            result, _, stderr = run_script(root)

            self.assertEqual(result, 1)
            self.assertIn("checksum mismatch for cuefn_1.2.3_linux_amd64", stderr)
    ...
    def test_fails_on_missing_os_arch_asset(self) -> None:
        with fixture(omit_artifact=("linux", "arm64", "Binary")) as root:
            ...
            self.assertEqual(result, 1)
            self.assertIn("missing expected binary asset cuefn_1.2.3_linux_arm64", stderr)
...
def run_script(root: Path) -> tuple[int, str, str]:
    stdout = io.StringIO()
    stderr = io.StringIO()
    with working_directory(root), github_repository("meigma/crossplane-cuefn"):
...
def fixture(
    *,
    signer: str = "meigma/crossplane-cuefn/.github/workflows/attest.yml",
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for goos, goarch in PLATFORMS:
            binary_name = f"cuefn_1.2.3_{goos}_{goarch}"
        ...
        if extra_binary:
            extra_name = "cuefn_1.2.3_freebsd_amd64"
...
os = "darwin"
arch = "amd64"
pattern = "cuefn_${{version}}_darwin_amd64"

[[packages.assets]]
os = "darwin"
arch = "arm64"
pattern = "cuefn_${{version}}_darwin_arm64"

[[packages.assets]]
os = "linux"
arch = "amd64"
pattern = "cuefn_${{version}}_linux_amd64"

[[packages.assets]]
os = "linux"
arch = "arm64"
pattern = "cuefn_${{version}}_linux_arm64"
