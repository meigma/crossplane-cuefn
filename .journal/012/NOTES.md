...
The hard Git gate passed without product subprocesses. `go-git` v5.19.1 opens normal and linked Worktrunk-style worktrees, requires whole-worktree cleanliness, packages only tracked module files, inherits a tracked repository-root `LICENSE` for nested modules, and records HEAD revision/committer time as CUE VCS annotations. Tests also prove ignored files stay out and generated-metadata collisions fail.
