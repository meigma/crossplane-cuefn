...
| 004 | 2026-06-29 | Dependency-aware local CUE loading, k8s example, and the module contract | in-progress | Shipped 5 PRs (#14–#18, all merged): dep-aware local loader + central-default registry + example on cue.dev/x/k8s.io + tests decoupled from the example. Now implementing module-contract v2 (single `out` root) + an importable contract module published to the CUE central registry. |
