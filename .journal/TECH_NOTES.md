...
## `cuefn check` — the static health gate (session 011, PRs #74/#76/#77, product v0.1.7)

The instance-free complement to `cuefn test`: three units (`fmt`, `vet`,
`xrd`) with the SAME PASS/FAIL/SEED/UPDATE lifecycle, CI detection, and exit
shape. Three-verb split (documented as a table in `cli.md`): check = static
module health, test = instance-driven behavior, validate = one XR vs schema.

- **Architecture:** `internal/check` core (bytes/values in, findings out) +
  `internal/textdiff` (`Lines` LCS diff + `Normalize`, extracted from
  testharness, shared by both harnesses); thin `internal/cli/check.go`
  adapter owns file walking (`fs.WalkDir` over `os.DirFS` — gosec G122
  rejects `os.ReadFile` on WalkDir callback paths) and golden I/O.
- **CRITICAL (error-reporting contract):** CUE evalv3 surfaces conflicts
  EAGERLY — regular, definition-nested, AND hidden-field conflicts all fail
  `render.LoadModule` at build, before any Validate walk (probed empirically;
  a load-ok-vet-fail module could not be constructed). So the CLI reports a
  load failure AS the vet unit's failure; `check.Vet` (the exact `cue vet
  -c=false` option set: Attributes/Definitions/Hidden + Concrete(false)) is a
  recursive backstop. Optional bottoms (`a?: 1 & 2`) pass BOTH our Vet and
  the real CLI — parity holds.
- **Golden semantics:** compare = byte-exact after `textdiff.Normalize` +
  stripping LEADING full-line `#` comments from both sides. Seed/`--update`
  write `check.GoldenBytes` (a `# Generated by cuefn check; DO NOT EDIT.`
  header naming the re-bless command). Headerless `cuefn generate --output`
  files therefore pass unchanged; to ADD the header to a matching golden,
  delete + reseed (`--update` is drift-only by design).
- **`cue mod tidy` is NOT in check** — tidy lives in CUE's
  `internal/mod/modload`, not importable at v0.16.1; docs point to
  `cue mod tidy --check`.
- **Dogfood:** moon `xrd-check` = `cuefn check --dir example/module --xrd
  example/xrd.yaml --ci` (was bash generate+diff); this is also the repo's
  only CUE fmt/vet gate for the example. `example/xrd.yaml` is machine-owned
  with the header.
- **Fmt parity** with bare `cue fmt` (no simplify) is pinned by fixtures
  verified against the mise-pinned CLI; re-verify if Dependabot #60
  (cuelang.org/go 0.16.1→0.17.1) merges — evalv3 eagerness assumptions too.
- **Release-ops gotchas (learned during v0.1.7):** select workflow runs by
  tag/displayTitle, never "latest run of workflow" (a recency wait-loop
  watched the previous release's distribute run to a false success);
